...
$projectPath = Join-Path $repoRoot 'CsProjFormatter.Cli\CsProjFormatter.Cli.csproj'
$testProjectPaths = @(
...
$skillSourcePath = Join-Path $repoRoot 'skills\csprojfmt'
...
if ($skillContent -notmatch '(?s)\A---\r?\nname:\s*csprojfmt\r?\ndescription:\s*.+?\r?\n---') {
    throw "The csprojfmt skill has invalid or unexpected frontmatter."
}

New-Item -ItemType Directory -Path $OutputDirectory -Force | Out-Null

$packageFileName = "csprojfmt-$packageVersionText.zip"
...
$stagingRoot = Join-Path $OutputDirectory ".csprojfmt-$packageVersionText-$temporaryId.tmp"
...
try {
    ...
    $stagedSkillPath = Join-Path $stagingRoot 'csprojfmt'
    ...
    Copy-Item `
        ...
        -Destination (Join-Path $cliPayloadPath 'CsProjFormatter-LICENSE.txt')
    ...
    Write-Host "CsProjFormatter skill package: $packagePath"
