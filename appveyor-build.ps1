...
function New-VsixArtifact {
    ...
    $artifactPath = Join-Path $OutputDirectory "CsProjFormatter-v$Version.vsix"
...
function Invoke-Build {
    ...
    Invoke-NativeCommand 'dotnet' @(
        'build',
        'CsProjFormatter\CsProjFormatter.csproj',
        '-c', 'Release',
        '-p:DeployExtension=false',
        '-p:ZipPackageCompressionLevel=normal'
    )

    Invoke-NativeCommand 'dotnet' @('build', 'CsProjFormatter.Cli\CsProjFormatter.Cli.csproj', '-c', 'Release')
    Invoke-NativeCommand 'dotnet' @('build', 'CsProjFormatter.CliTests\CsProjFormatter.CliTests.csproj', '-c', 'Release')
    Invoke-NativeCommand 'dotnet' @('build', 'CsProjFormatterTests\CsProjFormatterTests.csproj', '-c', 'Release')
    Invoke-NativeCommand 'dotnet' @(
        'test',
        'CsProjFormatterTests\CsProjFormatterTests.csproj',
        '-c', 'Release',
        '--no-build'
    )
    Invoke-NativeCommand 'dotnet' @(
        'test',
        'CsProjFormatter.CliTests\CsProjFormatter.CliTests.csproj',
        '-c', 'Release',
        '--no-build'
    )
